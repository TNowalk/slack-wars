...
// TODO: Sector cannot have self as neighbor
// TODO: Neighbor list in sectors needs to be unique
                    ...
                    Outpost.remove({}, (err) => {
                      ...
                      Outpost.create(newOutposts, (err, docs) => {
                        ...
                        for (let i = 0; i < outposts.length; i++) {
                          ...
                          // Add outpost to the universe
                          universe.outposts.push(outposts[i]);
                        }
                        ...
                        async.parallel(queue, (err) => {
                          ...
                          Player.remove({}, (err) => {
                            ...
                            logger.info('Whipped out all life');
...
Universe.count((err, count) => {
  if (err) handleError(err);

  if (count) {
    prompt.get([{
      name: 'force',
      description: 'Found existing universe, continue and destroy? [y/n]',
      pattern: /[yes|no|y|n]/,
    }], (err, result) => {
      if (err) handleError(err);

      if (['y', 'yes'].indexOf(result.force) !== -1) {
        bigBang(count);
      } else {
        logger.warn('Big Bang Aborted');
        process.exit(0);
      }
    });
  } else {
