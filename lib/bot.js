...
class Bot {
  ...
  handleMessage(message) {
    ...
    if (message.type === 'message' && message.text) {
      this.game.command(message.user, message.event, message.text).then((responses) => {
        if (responses) {
        ...
        logger.error('Error responding to message', message, err);
      });
    }
  }

  startGame(payload) {
    this.game = new Game(payload.team.id);

    // Init Game
    this.game.init().then((res) => {
      this.game.setLookup(this.lookup);

      logger.info(`Universe Loaded: ${res.clusters.length} Clusters, ` +
        `${res.sectors.length} Sectors, ${res.outposts.length} Outposts`);
    }, (err) => {
      logger.error('Error initializing universe', err);
      process.exit(1);
    });
  }
